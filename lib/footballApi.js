...
async function apiFetch(endpoint) {
  const res = await fetch(API_BASE + endpoint, { headers: { 'x-rapidapi-key': API_KEY, 'x-rapidapi-host': 'v3.football.api-sports.io' } })
...
export async function fetchLineup(id) { const d = await apiFetch('/fixtures?id=' + id); return d.response?.[0]?.lineups || [] }
export async function fetchPlayerStats(id) { const d = await apiFetch('/players?fixture=' + id); return d.response || [] }
...
export async function fetchOdds(id) { const d = await apiFetch('/odds?fixture=' + id); return d.response || [] }
