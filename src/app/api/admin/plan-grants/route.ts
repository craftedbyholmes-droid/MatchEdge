...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error: insertError } = await supabase.from("admin_plan_grants").insert({
      user_id: userId,
    ...
    const { data: resolved, error: resolvedError } = await supabase.rpc("get_effective_plan_key", {
    ...
    return NextResponse.json({
      ok: true,
      resolvedPlan: String(resolved || "free").toLowerCase(),
    });
  } catch (error) {
    return NextResponse.json(
