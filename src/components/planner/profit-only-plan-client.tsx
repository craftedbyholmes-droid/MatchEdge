...
import { formatEventDateTime, getCountdownMeta, getEventTimestamp } from "@/lib/time-display";
import { CopySearchControls } from "@/components/opportunities/copy-search-controls";

type BankrollMode = "per_bet" | "across_plan";
type SortMode = "highest_profit_percent" | "best_gbp_return" | "lowest_outlay";
type EffectivePlan = "free" | "premium" | "pro";
...
function countdownClasses(status: "urgent" | "soon" | "normal" | "started" | "none") {
  if (status === "urgent") return "border-rose-500/30 bg-rose-500/10 text-rose-100";
  if (status === "soon") return "border-amber-500/30 bg-amber-500/10 text-amber-100";
  if (status === "normal") return "border-cyan-400/20 bg-cyan-500/10 text-cyan-100";
...
export function ProfitOnlyPlanClient({
  ...
  const premiumOrAbove = effectivePlan === "premium" || effectivePlan === "pro";
  ...
  const rows = useMemo(() => {
    ...
    return mapped.sort((a, b) => {
      const timeDiff = getEventTimestamp(a.opportunity.eventDate) - getEventTimestamp(b.opportunity.eventDate);
      if (timeDiff !== 0) return timeDiff;

      if (sortMode === "best_gbp_return") return b.metrics.maxProfit - a.metrics.maxProfit;
      if (sortMode === "lowest_outlay") return a.outlay - b.outlay;
          ...
          grouped.map(([label, dayRows]) => (
            <div key={label} className="space-y-4">
              ...
              {dayRows.map(({ opportunity, state, metrics, outlay }) => {
                const countdown = getCountdownMeta(opportunity.eventDate);
                        ...
                        <div className="mt-3 flex flex-wrap gap-3">
                          {premiumOrAbove ? (
                            <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-2 text-sm text-cyan-100">
                              {formatEventDateTime(opportunity.eventDate)}
                            </div>
                          ) : null}

                          {effectivePlan === "premium" ? (
                            <div className={`rounded-2xl border px-4 py-2 text-sm ${countdownClasses(countdown.status)}`}>
                              {countdown.label}
                            </div>
                          ) : null}
