...
function computeSelectionMetrics(opportunity: Opportunity, maxOutlay: number) {
  const totalOriginalStake = opportunity.stakePlan.reduce((sum, leg) => sum + leg.stake, 0) || 1;
      ...
      opportunities.map((opportunity) => [
        opportunity.id,
        {
          ...
          maxOutlay: Math.max(
            ...
            Number(
              (
                opportunity.stakePlan.reduce((sum, leg) => sum + leg.stake, 0) || 25
              ).toFixed(2)
            )
          ),
        },
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          ...
          <div className="mt-4 text-3xl font-semibold">
            £{planSummary.minProfit.toFixed(2)} to £{planSummary.maxProfit.toFixed(2)}
          </div>
          <div className="mt-3 text-sm text-slate-400">
            Active combinations: {planSummary.count} • Completed: {planSummary.completedCount}
          </div>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0 flex-1">
                <div className="text-xs uppercase tracking-[0.3em] text-cyan-300">
                  {opportunity.sport}{opportunity.marketLabel ? ` • ${opportunity.marketLabel}` : ""} • {bucket === "qualifying" ? "Welcome/Qualifying" : "Profit"}
                </div>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-[220px_1fr]">
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm text-slate-300">
                  Completion: {rowState.completed ? "✓ Completed" : "Pending"}
                </div>
              </div>
                      ...
                      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                        <div>
                          <div className="text-sm">
                            <span className="font-medium">{leg.outcome}</span> • {leg.bookmaker}
                          </div>
                          <div className="mt-2 text-sm text-slate-400">
                            Current odds: {formatOdds(leg.odds)} • Stake: £{leg.adjustedStake.toFixed(2)}
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          >
                            {clicked ? `✓ ${leg.bookmaker} opened` : `Open ${leg.bookmaker}`}
                          </button>
                        </div>
                      </div>
                    </div>
