...
import { ALERT_SPORT_OPTIONS } from "@/lib/alerts-options";
...
function Toggle({ label, enabled, setEnabled }: { label: string; enabled: boolean; setEnabled: (value: boolean) => void }) {
  return (
    <button
      type="button"
      onClick={() => setEnabled(!enabled)}
      className={`rounded-2xl border px-4 py-3 text-sm text-left ${
        enabled ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-100" : "border-white/10 bg-slate-950 text-white"
      }`}
    >
