...
export default function PlanGate({ requiredPlan, currentPlan, children, leagueName = null }) {
  ...
  if (loading && current === 0) {
    return (
      <div style={{ background: '#13131a', border: '1px solid #2a2a3a', borderRadius: '10px', padding: '32px', margin: '16px 0', minHeight: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: '40px', height: '40px', borderRadius: '50%', border: '3px solid #1c1c28', borderTop: '3px solid #0F6E56', animation: 'me-spin 0.9s linear infinite' }} />
      </div>
    )
  }
