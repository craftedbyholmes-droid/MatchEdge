  ...
  return (
    <div style={{ width: '100%', maxWidth: '520px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '12px', fontWeight: 700 }}>
        <span style={{ color: '#00C896' }}>{homeTeam} {formation_home && formation_home !== 'None' ? '(' + formation_home + ')' : ''}</span>
        <span style={{ color: '#4d9fff' }}>{awayTeam} {formation_away && formation_away !== 'None' ? '(' + formation_away + ')' : ''}</span>
      </div>
      <svg viewBox='0 0 100 110' style={{ width: '100%', background: '#1a3a1a', borderRadius: '8px', border: '1px solid #2A3441' }}>
        {/* Pitch markings */}
        ...
        {allPlayers.map((p, i) => (
          <g key={i}>
            <circle
              ...
              cy={p.y}
              r='4.2'
              fill={p.side === 'home' ? '#00C896' : '#4d9fff'}
      ...
      <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginTop: '8px', fontSize: '11px', color: '#484F58' }}>
        <span><span style={{ color: '#00C896' }}>●</span> {homeTeam}</span>
