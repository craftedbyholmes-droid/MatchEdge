  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <p className="mt-2 text-sm text-slate-400">
        This panel helps you confirm whether admin upgrades are actually resolving to the expected effective plan.
      </p>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        ...
        <div>
          <div className="text-xs uppercase tracking-[0.28em] text-cyan-300">Billing profiles</div>
          <div className="mt-3 space-y-3">
            {billing.length ? (
              billing.map((row) => (
                <div key={row.user_id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                  ...
                  <div className="mt-2 text-xs text-slate-500">
                    Billing plan: {(row.current_plan_key || "free").toUpperCase()}
                  </div>
                  <div className="mt-1 text-xs text-slate-500">
                    Status: {row.billing_status || "n/a"}
                  </div>
                  <div className="mt-1 text-xs text-slate-500">
                    Period end: {row.current_period_end ? new Date(row.current_period_end).toLocaleString() : "n/a"}
                  </div>
                </div>
              ))
            ) : (
