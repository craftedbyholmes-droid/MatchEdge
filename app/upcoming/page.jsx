...
export default function UpcomingPage() {
  ...
  if (plan === 'free') return (
    <div style={{ paddingBottom: '60px' }}>
      ...
      <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '24px' }}>Early engine scores and predictions across 6 leagues.</p>
    ...
    <div style={{ paddingBottom: '60px' }}>
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          ...
          <p style={{ color: '#6b7280', fontSize: '13px', marginTop: '4px' }}>Real odds and engine predictions. Click any match for full markets.</p>
        </div>
        <Link href='/competitions' style={{ background: '#1c1c28', border: '1px solid #2a2a3a', color: '#9ca3af', padding: '8px 14px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, textDecoration: 'none' }}>Browse All Competitions</Link>
      </div>
      ...
      {loading ? <LoadingSpinner message='Loading fixtures...' /> : activeCategory !== 'top_leagues' ? (
        <div style={{ background: '#13131a', border: '1px solid #2a2a3a', borderRadius: '8px', padding: '32px', textAlign: 'center' }}>
          ...
          <div style={{ color: '#6b7280', fontSize: '13px', marginBottom: '16px' }}>Browse this week fixtures for all competitions in this category.</div>
          <Link href='/competitions' style={{ background: '#0F6E56', color: '#fff', padding: '10px 24px', borderRadius: '6px', fontWeight: 700, fontSize: '14px', textDecoration: 'none' }}>Browse Competitions</Link>
        </div>
              ...
              <div key={match.fixture_id} style={{ background: '#13131a', border: '1px solid ' + (badge ? badge.colour + '40' : '#2a2a3a'), borderTop: 'none', borderRadius: isLast && !isOpen ? '0 0 8px 8px' : '0', overflow: 'hidden' }}>
                <div onClick={() => toggle(match.fixture_id)} style={{ padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ color: '#6b7280', fontSize: '14px' }}>{isOpen ? 'v' : '>'}</span>
                  </div>
                </div>
                {isOpen && (
                  <div style={{ borderTop: '1px solid #2a2a3a', padding: '16px' }}>
                    ...
                    {preds ? (
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <MarketRow title='MATCH RESULT' items={preds.match_result} colour='#0F6E56' />
                          ...
                          <div style={{ marginBottom: '10px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                              {preds.anytime_scorer.map((p, i) => (
                                <div key={i} style={{ background: '#1c1c28', border: '1px solid #2a2a3a', borderRadius: '6px', padding: '8px 10px', minWidth: '100px' }}>
                        ...
                        <div style={{ fontSize: '11px', color: '#4b5563', marginTop: '4px' }}>Odds sourced from bookmakers. Always check current prices.</div>
                      </div>
                    ) : <div style={{ color: '#6b7280', fontSize: '13px', marginBottom: '12px' }}>Run Score from Admin to generate predictions.</div>}
...
function MarketRow({ title, items, colour }) {
  return (
    <div style={{ marginBottom: '10px' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {items.map(p => (
          <div key={p.label} style={{ flex: '1 1 100px', background: p.signal === 'strong' ? colour + '20' : '#1c1c28', border: '1px solid ' + (p.signal === 'strong' ? colour + '60' : '#2a2a3a'), borderRadius: '6px', padding: '8px', textAlign: 'center' }}>
