...
export async function GET(request) {
  ...
  try {
    ...
    // Get all goal events for these fixtures
    const { data: goalEvents } = await supabaseAdmin
      .from('match_event_log')
      .select('fixture_id, player_name, event_type')
      .in('fixture_id', fixtureIds)
    ...
    for (const g of (goalEvents || [])) {
      if (!goalMap[g.fixture_id]) goalMap[g.fixture_id] = []
      goalMap[g.fixture_id].push((g.player_name || '').toLowerCase())
    }
    ...
    for (const pick of unsettled) {
      ...
      if (market === 'match_result') {
        // Word-by-word matching - any significant word from team name in selection
        ...
        outcome = totalGoals > 2.5 ? 'win' : 'loss'

      } else if (market === 'under_25') {
        outcome = totalGoals < 2.5 ? 'win' : 'loss'

      } else if (market === 'anytime_scorer') {
        const scorers = goalMap[pick.fixture_id] || []
        if (!scorers.length) {
          // No event data yet - leave pending
          continue
        }
        // Extract player name from selection - remove 'Anytime Scorer' suffix
        const playerName = sel.replace('anytime scorer', '').trim()
        // Match any word of player name against scorer list
        const nameParts = playerName.split(' ').filter(w => w.length > 2)
        const scored = scorers.some(s => nameParts.some(part => s.includes(part)))
        outcome = scored ? 'win' : 'loss'
      }
    ...
    for (const [persona, t] of Object.entries(personaTotals)) {
      const { data: cur } = await supabaseAdmin
        .from('persona_season').select('*').eq('persona', persona).single()
      const c = cur || { total_picks:0, wins:0, losses:0, voids:0, total_staked:0, profit_loss:0 }
      const newPL     = Math.round((Number(c.profit_loss || 0) + t.pl) * 100) / 100
      const newPicks  = (c.total_picks || 0) + t.picks
      const newWins   = (c.wins || 0) + t.wins
      const newStaked = Number(c.total_staked || 0) + t.staked
      await supabaseAdmin.from('persona_season').upsert({
        ...
        roi:          newStaked > 0 ? Math.round((newPL / newStaked) * 100) / 100 : 0,
