...
export default function TipstersPage() {
  ...
  return (
    <div style={{ paddingBottom: '60px' }}>
      ...
      <div style={{ background: '#161B22', border: '1px solid #00C89660', borderRadius: '8px', padding: '14px 18px', marginBottom: '24px' }}>
        <div style={{ fontSize: '14px', fontWeight: 700, color: '#00C896', marginBottom: '6px' }}>When do picks go up?</div>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '24px' }}>
        {Object.entries(PERSONA_META).map(([id, meta]) => {
          ...
          return (
            <div key={id} style={{ flex: '1 1 260px', background: '#161B22', border: '1px solid ' + meta.colour + '40', borderRadius: '8px', padding: '18px' }}>
              ...
              <div style={{ color: '#8B949E', fontSize: '12px', marginBottom: '12px' }}>{meta.bio}</div>
              <div style={{ display: 'flex', gap: '16px' }}>
                <div><div style={{ fontSize: '18px', fontWeight: 800 }}>{s.wins}/{s.total_picks}</div><div style={{ fontSize: '11px', color: '#484F58' }}>W/Total ({wr}%)</div></div>
                <div><div style={{ fontSize: '18px', fontWeight: 800, color: plColour(pl) }}>{pl >= 0 ? '+' : ''}£{Math.abs(pl).toFixed(2)}</div><div style={{ fontSize: '11px', color: '#484F58' }}>Season P+L</div></div>
              </div>
            </div>
          )
        })}
      ...
      {plan === 'free' ? (
        ...
        <>
          ...
          {picks.length === 0 ? (
            <div style={{ background: '#161B22', border: '1px solid #2A3441', borderRadius: '8px', padding: '24px', textAlign: 'center', marginBottom: '24px' }}>
              <div style={{ fontSize: '14px', color: '#484F58', marginBottom: '6px' }}>No picks generated yet</div>
              <div style={{ fontSize: '12px', color: '#484F58' }}>Picks are generated automatically when the next matchday fixtures are scored.</div>
            </div>
          ) : (
                ...
                return (
                  <div key={id} style={{ marginBottom: '20px' }}>
                    ...
                    {personaPicks.map(pick => (
                      <div key={pick.pick_id} style={{ background: '#161B22', border: '1px solid ' + meta.colour + '30', borderRadius: '8px', padding: '14px 16px', marginBottom: '8px' }}>
                        {/* Match name - most important line */}
                        <div style={{ fontWeight: 700, fontSize: '15px', marginBottom: '6px', color: '#E6EDF3' }}>
                          {getMatchName(pick)}
                        </div>
                        {/* League + kickoff */}
                        {(pick.league || pick.kickoff_time) && (
                          <div style={{ fontSize: '11px', color: '#484F58', marginBottom: '8px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '6px' }}>
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            <span style={{ fontWeight: 600, color: '#E6EDF3' }}>{pick.selection}</span>
                            <span style={{ color: '#00C896', fontWeight: 700 }}>@ {pick.odds_fractional}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: '#484F58' }}>
                        ...
                        {/* AI tip */}
                        {pick.tip_text && (
                          <div style={{ fontSize: '13px', color: '#484F58', fontStyle: 'italic', marginBottom: '10px', paddingLeft: '8px', borderLeft: '2px solid ' + meta.colour + '40' }}>
                            {pick.tip_text}
                          </div>
                        )}
                        {/* Bookmaker links */}
                        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', paddingTop: '8px', borderTop: '1px solid #1E2530' }}>
                          <span style={{ fontSize: '10px', color: '#484F58', marginRight: '4px', lineHeight: '26px' }}>Bet with:</span>
                          {BOOKMAKERS.map(bm => (
                            <a key={bm} href='#' target='_blank' rel='noopener noreferrer' style={{ background: '#1E2530', border: '1px solid #2A3441', color: '#8B949E', padding: '3px 8px', borderRadius: '4px', fontSize: '11px', textDecoration: 'none' }}>{bm}</a>
                          ))}
                        </div>
                      </div>
                    ))}
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
            {Object.entries(PERSONA_META).map(([id, meta]) => (
              <button key={id} onClick={() => setActiveTab(id)} style={{ padding: '6px 16px', background: activeTab === id ? meta.colour : '#1E2530', color: activeTab === id ? '#fff' : '#8B949E', border: '1px solid ' + (activeTab === id ? meta.colour : '#2A3441'), borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                {meta.name}
              </button>
            ))}
          </div>
          {loading ? <LoadingSpinner message='Loading history...' /> : getHistory(activeTab).length === 0 ? (
            <div style={{ color: '#484F58', fontSize: '14px' }}>No settled picks yet. History builds after each matchday settles.</div>
          ) : (
            getHistory(activeTab).map(pick => (
              <div key={pick.pick_id} style={{ background: '#161B22', border: '1px solid #2A3441', borderRadius: '6px', padding: '12px 14px', marginBottom: '6px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '13px', marginBottom: '2px' }}>{getMatchName(pick)}</div>
                    <div style={{ fontSize: '12px', color: '#8B949E' }}>{pick.selection} @ {pick.odds_fractional}</div>
                    <div style={{ fontSize: '11px', color: '#484F58', marginTop: '2px' }}>{pick.pick_date} - {pick.league || ''}</div>
                  </div>
