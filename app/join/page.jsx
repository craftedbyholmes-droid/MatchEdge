...
export default function JoinPage() {
  ...
  const inputStyle = { width: '100%', padding: '10px 12px', background: '#1E2530', border: '1px solid #2A3441', borderRadius: '6px', color: '#E6EDF3', fontSize: '16px', marginBottom: '12px' }
  const checkRow = { display: 'flex', gap: '10px', alignItems: 'flex-start', marginBottom: '14px', fontSize: '13px', color: '#8B949E', lineHeight: '1.5' }
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '40px auto', padding: '0 16px' }}>
      ...
      <p style={{ color: '#484F58', fontSize: '14px', marginBottom: '24px' }}>Free to join. No card required.</p>

      <div style={{ background: '#161B22', border: '1px solid #f59e0b40', borderRadius: '8px', padding: '14px 16px', marginBottom: '20px', fontSize: '13px', color: '#f59e0b' }}>
      ...
      {message ? (
        <div style={{ background: '#00C89620', border: '1px solid #00C896', borderRadius: '8px', padding: '16px', color: '#00C896', fontSize: '14px' }}>{message}</div>
      ) : (
        <>
          ...
          <div style={{ margin: '8px 0 16px' }}>
            ...
            <label style={checkRow}>
              ...
              <span>I have read and accept the <Link href='/terms' style={{ color: '#00C896', textDecoration: 'underline' }}>Terms and Conditions</Link> and <Link href='/privacy' style={{ color: '#00C896', textDecoration: 'underline' }}>Privacy Policy</Link>.</span>
            </label>
            <label style={checkRow}>
              <input type='checkbox' checked={aware} onChange={e => setAware(e.target.checked)} style={{ marginTop: '2px', flexShrink: 0 }} />
              <span>I understand that sports betting involves risk. MatchEdge promotes responsible gambling. If gambling is causing you harm, please visit <a href='https://www.begambleaware.org' target='_blank' rel='noopener noreferrer' style={{ color: '#00C896', textDecoration: 'underline' }}>BeGambleAware.org</a> or call <strong>0808 8020 133</strong> (free, 24/7).</span>
            </label>
          </div>

          {error && <div style={{ color: '#ef4444', fontSize: '13px', marginBottom: '12px', padding: '10px', background: '#ef444410', borderRadius: '6px' }}>{error}</div>}

          <button onClick={handleSignUp} disabled={loading} style={{ width: '100%', padding: '12px', background: loading ? '#1E2530' : '#00C896', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 600, fontSize: '15px', cursor: loading ? 'not-allowed' : 'pointer' }}>
            {loading ? 'Creating account...' : 'Create Free Account'}
          </button>

          <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '13px', color: '#484F58' }}>
            Already have an account? <Link href='/account' style={{ color: '#00C896' }}>Sign in</Link>
          </p>
        </>
      )}

      <div style={{ marginTop: '32px', padding: '14px 16px', background: '#161B22', borderRadius: '8px', fontSize: '12px', color: '#484F58', textAlign: 'center', lineHeight: '1.6' }}>
