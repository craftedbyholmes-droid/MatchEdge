  ...
  return (
    <div style={{ paddingBottom: '60px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0a2463 0%, #1b4332 100%)', borderRadius: '10px', padding: '24px', marginBottom: '24px', textAlign: 'center' }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--gold)', letterSpacing: '2px', marginBottom: '8px' }}>FIFA</div>
        <h1 style={{ fontSize: '28px', fontWeight: 900, marginBottom: '6px' }}>World Cup 2026</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '12px' }}>USA · Canada · Mexico · 11 June — 19 July 2026</p>
      ...
      {picks.length > 0 && (
        <div style={{ marginBottom: '24px' }}>
          ...
          {plan === 'free' ? (
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px', textAlign: 'center' }}>
              <div style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '12px' }}>Upgrade to see World Cup tipster selections</div>
              <a href='/pricing' style={{ background: 'var(--gold)', color: '#0B0E11', padding: '8px 20px', borderRadius: '6px', fontWeight: 700, fontSize: '13px' }}>Try Today £1.99</a>
            </div>
          ) : (
            picks.map(pick => (
              <div key={pick.pick_id} style={{ background: 'var(--card)', border: '1px solid ' + (PERSONA_COLOUR[pick.persona] || 'var(--border)') + '30', borderRadius: '8px', padding: '14px 16px', marginBottom: '8px' }}>
                ...
                {pick.tip_text && <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontStyle: 'italic', marginTop: '4px' }}>{pick.tip_text}</div>}
              </div>
            ))
          )}
        </div>
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '32px', textAlign: 'center' }}>
          <div style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '8px' }}>Group stage data will populate as the tournament approaches.</div>
          <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Tournament starts 11 June 2026. Check back closer to the event.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          {groups.map(group => (
            <div key={group.group_id} style={{ flex: '1 1 300px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ background: 'var(--card-raised)', padding: '10px 14px', fontWeight: 700, fontSize: '14px', borderBottom: '1px solid var(--border)' }}>
                {group.group_name}
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {(group.teams || []).sort((a,b) => b.points - a.points || (b.goals_for-b.goals_against) - (a.goals_for-a.goals_against)).map((team, i) => (
                    <tr key={team.team_id} style={{ borderBottom: '1px solid var(--card-raised)', background: i < 2 ? 'var(--primary)10' : 'transparent' }}>
                      <td style={{ padding: '8px 14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {i < 2 && <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--primary)', flexShrink: 0, display: 'inline-block' }} />}
                        <span style={{ fontWeight: i < 2 ? 600 : 400 }}>{team.name}</span>
                        <span style={{ fontSize: '10px', color: CONF_COLOURS[team.confederation] || 'var(--text-muted)', background: (CONF_COLOURS[team.confederation] || 'var(--text-muted)') + '20', padding: '1px 5px', borderRadius: '8px' }}>{team.confederation}</span>
                      </td>
                      <td style={{ padding: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>{team.played}</td>
                      <td style={{ padding: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>{team.wins}</td>
                      <td style={{ padding: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>{team.draws}</td>
                      <td style={{ padding: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>{team.losses}</td>
                      <td style={{ padding: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>{(team.goals_for - team.goals_against) >= 0 ? '+' : ''}{team.goals_for - team.goals_against}</td>
                      <td style={{ padding: '8px 14px', textAlign: 'center', fontWeight: 800, color: 'var(--text)' }}>{team.points}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {matches.length > 0 && (
        <div style={{ marginTop: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '16px' }}>
            {['all', ...stages].map(s => (
              <button key={s} onClick={() => setActiveStage(s)} style={{ padding: '5px 12px', background: activeStage === s ? 'var(--primary)' : 'var(--card-raised)', color: '#fff', border: '1px solid ' + (activeStage === s ? 'var(--primary)' : 'var(--border)'), borderRadius: '20px', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}>
                {s === 'all' ? 'All Stages' : s}
              </button>
            ))}
          </div>
          {filteredMatches.map(match => (
            <div key={match.match_id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '12px 16px', marginBottom: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                ...
                {match.status === 'FT' ? (
                  ...
                  <div style={{ fontWeight: 800, fontSize: '18px', color: 'var(--win)' }}>{match.home_score} — {match.away_score}</div>
                ) : null}
              </div>
            </div>
          ))}
...
function CountdownBanner() {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--gold)40', borderRadius: '8px', padding: '16px', marginBottom: '24px', textAlign: 'center' }}>
      <div style={{ fontSize: '36px', fontWeight: 900, color: 'var(--gold)' }}>{days}</div>
      <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>days until kickoff — Mexico vs host nation opens the tournament</div>
    </div>
  )
}